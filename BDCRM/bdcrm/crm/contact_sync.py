...
def upsert_contact_from_common_payload(data):
    ...
    # A telecaller saving an outcome is a completed planner call even when
    # the shared contact has not been marked as "verified".  Verification is
    # contact-level CRM data; PlannerCallAssignment is work-level data.
    # Keeping these independent prevents a Not Connected / Follow Up result
    # from leaving BDCRM's daily pending total stale.
    activity_was_logged = str(data.get("contact_status") or "").strip().lower() in {
        "contacted", "not_connected", "follow_up"
    }
    if payload["is_verified"] or activity_was_logged:
            ...
            try:
                response = requests.post(
                    ...
                    headers={
                        ...
                        "X-Company-ID": str(get_current_company_id()),
                    },
...
def sync_assignment_to_peer(contact, assigned_user, assignment=None, planner_name=""):
    """Send one planner assignment to Marketing CRM and report its outcome."""
    urls = get_sync_assignment_target_urls()
    token = getattr(settings, "CONTACT_SYNC_API_TOKEN", "")
    if not urls or not token or not assigned_user or not assigned_user.email:
        return 0, 0, 1
    # Use the exact daily task generated by BDCRM, not the monthly average.
    daily_target = getattr(getattr(assignment, 'planner_task', None), 'target_count', 0) or 0
    ...
    payload = {
        ...
        # Marketing CRM has its own user table.  Send the already-hashed
        # password over the authenticated internal sync channel so a planner
        # user can sign in there with the same BDCRM password; no plaintext
        # password is sent or stored.
        "assigned_user_password_hash": assigned_user.password,
        "assigned_user_is_active": bool(assigned_user.is_active),
        "assigned_user_first_name": assigned_user.first_name or "",
        "assigned_user_last_name": assigned_user.last_name or "",
        ...
        # Exact BDCRM target for this assignment's scheduled day.
...
def send_assignment_to_peer(contact, assigned_user, assignment=None, planner_name=""):
    """Mirror a BDCRM planner assignment after the current transaction commits."""
    # Assignment sync is auxiliary; a CRM/network failure must never make the
    # BDCRM planner save fail with HTTP 500.
    try:
        transaction.on_commit(
            lambda: sync_assignment_to_peer(contact, assigned_user, assignment, planner_name)
        )

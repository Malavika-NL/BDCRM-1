...
const CHANNELS: Channel[] = ['calls', 'whatsapp', 'email', 'linkedin'];

/* ─── Inject global styles ─── */
const injectStyles = () => {
  const id = 'activity-planner-styles';
  ...
  el.textContent = `
    ...
    .ap-fade-up { animation: fadeInUp 0.4s ease both; }
    .ap-fade-in { animation: fadeIn 0.3s ease both; }
    .ap-scale-in { animation: scaleIn 0.3s ease both; }
    .ap-slide-right { animation: slideRight 0.3s ease both; }
    ...
    .ap-weekend-toggle:hover {
      transform: translateY(-1px);
    }

    .ap-scroll::-webkit-scrollbar { width: 5px; }
    .ap-scroll::-webkit-scrollbar-track { background: transparent; }
    .ap-scroll::-webkit-scrollbar-thumb { background: #cbd5e1; border-radius: 3px; }
    .ap-scroll::-webkit-scrollbar-thumb:hover { background: #94a3b8; }

    @media (max-width: 1024px) {
      .ap-step1-grid { grid-template-columns: 1fr !important; }
      .ap-step2-grid { grid-template-columns: 1fr !important; }
      .ap-member-grid { grid-template-columns: 1fr !important; }
      .ap-week-grid { grid-template-columns: repeat(2, 1fr) !important; }
    }
    @media (max-width: 768px) {
      .ap-calendar-layout { flex-direction: column !important; }
      .ap-sidebar { width: 100% !important; max-height: 300px; border-right: none !important; border-bottom: 1px solid #e2e8f0; }
      .ap-week-grid { grid-template-columns: 1fr !important; }
      .ap-stats-grid { grid-template-columns: repeat(2, 1fr) !important; }
      .ap-header-steps { display: none !important; }
      .ap-day-grid-7 { grid-template-columns: repeat(3, 1fr) !important; }
    }
    @media (max-width: 480px) {
      .ap-stats-grid { grid-template-columns: 1fr !important; }
      .ap-day-grid-7 { grid-template-columns: repeat(2, 1fr) !important; }
    }
  `;
...
export function ActivityPlannerPage() {
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: '#f8fafc', overflow: 'hidden', ...ff }}>
      ...
      <main style={{ flex: 1, display: 'flex', minHeight: 0 }}>
        ...
        {step === 3 && selectedPlanner ? (
          ...
          <div style={{ flex: 1, overflow: 'auto' }} className="ap-scroll">
            ...
            {/* ── STEP 1 ── */}
            {step === 1 && (
              <div className="ap-step1-grid ap-fade-up" style={{
                minHeight: '100%', padding: 28,
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                  ...
                  <div className="ap-card" style={{ padding: 24 }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                      <div>
                        <label style={{ fontSize: 11, fontWeight: 700, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.05em', display: 'block', marginBottom: 6, ...ff }}>
                          Plan name
                        </label>
                        <input className="ap-input" value={planName} onChange={e => setPlanName(e.target.value)} />
                      </div>
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                        <div>
                          ...
                          <select className="ap-select" value={month} onChange={e => setMonth(Number(e.target.value))}>
                            {MONTHS.map(m => <option key={m.value} value={m.value}>{m.label}</option>)}
                          </select>
                        </div>
                        <div>
                          ...
                          <input type="number" className="ap-input" value={year} onChange={e => setYear(Number(e.target.value))} />
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="ap-card" style={{ padding: 24 }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                      ...
                      <span style={{
                        ...
                        borderRadius: 20, border: '1px solid #c7d2fe', ...ff,
                      }}>
                        {workingWeekendDates.length} selected
                      </span>
                    </div>
                    <p style={{ fontSize: 12, color: '#94a3b8', margin: '0 0 14px', ...ff }}>
                      Weekdays are always working. Toggle any weekend to mark as a working day.
                    </p>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8 }}>
                      {weekendOptions.map(date => {
                        ...
                        return (
                          ...
                          >
                            <span style={{ fontWeight: 800, fontSize: 16 }}>{date.slice(8)}</span>
                            <span style={{ fontSize: 10, opacity: 0.7 }}>
                              {day.getDay() === 6 ? 'Sat' : 'Sun'}
                            </span>
                          </button>
                        );
                      })}
            ...
            {/* ── STEP 2 ── */}
            {step === 2 && (
              <div className="ap-step2-grid ap-fade-up" style={{
                minHeight: '100%', padding: 28,
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                  ...
                  <div className="ap-card" style={{ padding: 20 }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
                      <p style={{ fontSize: 11, fontWeight: 800, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.06em', margin: 0, ...ff }}>
                      ...
                      <button
                        ...
                        style={{
                          ...
                          background: 'transparent', border: 'none', cursor: 'pointer', ...ff,
                        }}
                      >
                        <Plus size={13} /> Add
                      </button>
                      ...
                      {members.map((m, i) => (
                        <div key={i} style={{
                          ...
                          <div style={{
                            ...
                            display: 'flex', alignItems: 'center', justifyContent: 'center',
                            fontWeight: 800, fontSize: 12, flexShrink: 0, ...ff,
                          }}>
                            {m.member_name?.charAt(0)?.toUpperCase() || '?'}
                          </div>
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <p style={{
                              fontSize: 12, fontWeight: 600, color: '#334155', margin: 0,
                              overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', ...ff,
                            }}>
                              {m.member_name || 'Unnamed agent'}
                            </p>
                            <p style={{ fontSize: 10, color: '#94a3b8', margin: '1px 0 0', ...ff }}>
                              {memberTotals(m)} activities · {dailyAvg(m)}/day
                            </p>
                          </div>
                        </div>
                      ))}
                  ...
                  <div style={{
                    ...
                    <p style={{ fontSize: 10, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#64748b', margin: '0 0 16px', ...ff }}>
                      Plan Summary
                    </p>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                      {[
                        ...
                        <div key={r.label} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13 }}>
                          <span style={{ color: '#64748b', ...ff }}>{r.label}</span>
                          <span style={{ fontWeight: 600, ...ff }}>{r.value}</span>
                        </div>
                      ))}
                      <div style={{
                        borderTop: '1px solid #334155', paddingTop: 10, marginTop: 4,
                        display: 'flex', justifyContent: 'space-between', fontSize: 13,
                      }}>
                        <span style={{ color: '#64748b', ...ff }}>Total activities</span>
                        <span style={{ fontWeight: 900, color: '#818cf8', fontSize: 16, ...ff }}>
                          {members.reduce((sum, m) => sum + memberTotals(m), 0)}
                        </span>
                      </div>
                    </div>
                ...
                {/* Right — agent config cards */}
                <div>
                  <div style={{ marginBottom: 20 }}>
                    <h2 style={{ fontSize: 20, fontWeight: 900, color: '#0f172a', margin: 0, ...ff }}>
                      Configure Agents
                    </h2>
                    <p style={{ fontSize: 13, color: '#94a3b8', margin: '4px 0 0', ...ff }}>
                      Set monthly channel targets for each team member
                    </p>
                  </div>

                  <div className="ap-member-grid" style={{
                    display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 16,
                  }}>
                    {members.map((m, i) => (
                      <div key={i} className="ap-card ap-fade-up" style={{
                        ...
                        <div style={{
                          ...
                          <div style={{
                            ...
                            background: 'linear-gradient(135deg, #4f46e5 0%, #7c3aed 100%)',
                            display: 'flex', alignItems: 'center', justifyContent: 'center',
                            fontWeight: 800, fontSize: 15, color: '#fff', flexShrink: 0, ...ff,
                          }}>
                            {m.member_name?.charAt(0)?.toUpperCase() || '?'}
                          </div>
                          <input
                            ...
                            style={{
                              ...
                              fontSize: 14, fontWeight: 700, color: '#0f172a', ...ff,
                            }}
                          />
                          {members.length > 1 && (
                            <button
                        ...
                        <div style={{ padding: 16 }}>
                          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                            {CHANNELS.map(ch => (
                              <div key={ch} style={{
                                ...
                                background: CHANNEL_META[ch].bg,
                                border: `1.5px solid ${CHANNEL_META[ch].border}`,
                              }}>
                                <div style={{
                                  display: 'flex', alignItems: 'center', gap: 5,
                                  fontSize: 10, fontWeight: 700, color: CHANNEL_META[ch].text,
                                ...
                                />
                                <p style={{ fontSize: 10, color: '#94a3b8', margin: '4px 0 0', ...ff }}>
                                  ~{workdayCount > 0 ? (m[`monthly_${ch}_target`] / workdayCount).toFixed(1) : 0}/day
                                </p>
                              </div>
                            ))}
                          ...
                          <div style={{
                            ...
                            <span style={{ fontSize: 12, color: '#64748b', fontWeight: 600, ...ff }}>
                              Total activities
                            </span>
                            <span style={{ fontSize: 16, fontWeight: 900, color: '#0f172a', ...ff }}>
                              {memberTotals(m)}
                            </span>
                          </div>
                        </div>
                  ...
                  <div style={{ display: 'flex', gap: 12, marginTop: 24 }}>
                    <button className="ap-btn-secondary" onClick={() => setStep(1)}>
                      <ChevronLeft size={15} />
                      Back
                    </button>
                    <button
